...
CRYPTO_CONFIG_DIR=/opt/share/fabric/crypto-config/
ORG1_DIR=${CRYPTO_CONFIG_DIR}/peerOrganizations/org1
ORG1_CA_DIR=${ORG1_DIR}/peers/peer0.org1
ORG1_USER_DIR=${ORG1_DIR}/users/Admin@org1/msp

ORG2_DIR=${CRYPTO_CONFIG_DIR}/peerOrganizations/org2
ORG2_CA_DIR=${ORG2_DIR}/peers/peer0.org2
ORG2_USER_DIR=${ORG2_DIR}/users/Admin@org2/msp

ORDERER_DIR=${CRYPTO_CONFIG_DIR}/ordererOrganizations/orgorderer1
ORDERER_MSP_DIR=${ORDERER_DIR}/orderers/orderer0.orgorderer1
...
function make_network_card(){
    ...
    set -e
    composer card create -p `eval echo '$'{"ORG$1_NETWORK_FILE"}` -u ${USER_NAME} -c `eval echo '$'{"ORG$1_USER_DIR"}`/signcerts/Admin@org$1-cert.pem -k `eval echo '$'{"ORG$1_USER_DIR"}`/keystore/*_sk -r PeerAdmin -r ChannelAdmin -f ${CARD_NAME}.card
